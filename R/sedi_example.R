...
BR_2 <- BaseRate(a = a, b = c, c = b, d = d)

H_1 <- rep(HitRate(a = a, c = c), 20)
H_2 <- rep(HitRate(a = a, c = b), 20)

F_1 <- FalseAlarm(b = b, d = d)
F_2 <- FalseAlarm(b = c, d = d)

TSS_1 <- TSS(a = a, b = b, c = c, d = d)
TSS_2 <- TSS(a = a, b = c, c = b, d = d)

ORSS_1 <- ORSS(a = a, b = b, c = c, d = d)
ORSS_2 <- ORSS(a = a, b = c, c = b, d = d)
...
SEDI_2 <- sedi(a = a, b = c, c = b, d = d)[[2]] # We can think about marking those cases where either b or c are zero somehow
SEDI_2_W <- sedi(a = a, b = c, c = b, d = d)[[1]]
...
legend("bottomleft", legend = c("B1", "B2"), col="black", lty = c("dashed", "dotted"), lwd = 2) # optional legend